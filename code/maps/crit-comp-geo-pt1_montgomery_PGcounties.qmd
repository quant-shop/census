...
ggplot(md_tracts) +
  ...
  labs(title = "Estimated Black Population by Census Tract in MD (2023)",
       fill = "Population") +
  theme_minimal()
```
...
ggplot(md_tracts) +
  ...
  scale_fill_viridis_c(option = "magma", 
                       na.value = "grey50",
                       labels = comma) +
  labs(title = "Estimated Non-Black Population by Census Tract in MD (2023)",
       fill = "Population") +
  theme_minimal()
```
...
We first make a `md_black_pop` data frame.

```{r, output=F, message=F, warning=F}
# Get Black population by tract in DC
md_black_pop <- get_acs(
  geography = "tract",
...
We then grab the total population by tract in MD, we will call it `md_total_pop`.

```{r, output=F, message=F, warning=F}
# Get total population by tract in DC
md_total_pop <- get_acs(
  geography = "tract",
...
```{r, message=F, warning=F}
md_combined <- left_join(md_black_pop, md_total_pop, by = "GEOID") %>%
...
We then calculate the proportion of Black people in all MD tracts.

```{r, message=F, warning=F}
total_black <- sum(md_combined$estimate_black, na.rm = TRUE) # this is represented by big B 
total_nonblack <- sum(md_combined$estimate_nonblack, na.rm = TRUE) # this is represented by big O
...
We see that     of all MD tracts have a population of more than     percent Black.

Finally, we calculate $D$.

```{r, message=F, warning=F}
dissimilarity_md = 0.5 * sum(abs(
  (md_combined$estimate_black / total_black) -
  (md_combined$estimate_nonblack / total_nonblack)
), na.rm = TRUE)
```
...
Values between roughly 0.3 to 0.6 indicate moderate segregation; above 0.6 is high segregation. In this instance, the model result suggests residential segregation by race in MD. We close by visualizing our result and the potential [dividing wall](#conceptual-model).

```{r, message=F, warning=F}
# we need to manually convert our combined data frame to include simple features
md_combined <- st_as_sf(md_combined)

ggplot(md_combined) +
  geom_sf(aes(fill = proportion_black), color = "white") +
  scale_fill_viridis_c(option = "plasma", direction = -1) +
  labs(title = "Proportion of Black Residents by Census Tract in MD",
...
union_black <- md_combined %>%
  filter(majority_black) %>%
  summarise(geometry = st_union(geometry))

union_nonblack <- md_combined %>%
  filter(!majority_black) %>%
  summarise(geometry = st_union(geometry))
```
...
```{r, message=F, warning=F}
ggplot() +
  geom_sf(data = md_combined, aes(fill = majority_black), 
          color = "grey40", 
          alpha = 0.5) +
...
For MD, we'll use `geography = "county"` and then make our way down to `tracts`.
...
We see that Baltimore County, MD has the highest proportion of Black people in the state, but only by a small margin. Alternatively, in the code below, we see that Garrett, Carroll, and Queen Anne's counties have the lowest proportion of Black people in the state.
...
- Examine counties with the highest proportion of Black residents and gather a measure of residential segregation,
...
I will select my current county: Montgomery County, MD for further investigation.

```{r, output=F, message=F, warning=F}
montgomery_tracts <- get_acs(
  geography = "tract",
  variables = c(
...
```{r, message=F, warning=F}
ggplot(montgomery_tracts) +
  geom_sf(aes(fill = blackE), color = NA) +
  scale_fill_viridis_c(option = "magma", 
                       na.value = "grey50", 
                       labels = comma) +
  labs(title = "Estimated Black Population by Census Tract in Montgomery County, MD (2023)",
       fill = "Population") +
  theme_minimal()
```
...
```{r, message=F, warning=F}
montgomery_tracts <- montgomery_tracts %>% 
...
We can calculate the proportion of Black people in all Montgomery tracts.
...
Approximately 0.01 percent of tracts have a Black population greater than or equal to 60 percent in MD.

Finally, we calculate $D$ for the county.

```{r, message=F, warning=F}
montgomery_total_black <- sum(montgomery_tracts$blackE, na.rm = TRUE)
montgomery_total_nonblack <- sum(montgomery_tracts$nonblackE, na.rm = TRUE)

dissimilarity_montgomery <- 0.5 * sum(abs(
  (montgomery_tracts$blackE / total_black) - 
  (montgomery_tracts$nonblackE / total_nonblack)
), na.rm = TRUE)
```
...
```{r, message=F, warning=F}
dissimilarity_montgomery
```

Values between roughly 0.3 to 0.6 indicate moderate segregation; above 0.6 is high segregation. In this instance, the model result suggests insignificant residential segregation by race in Montgomery County. We close by visualizing our result and the potential dividing wall.
...
```{r, message=F, warning=F}
# Union geometries by group
montgomery_union_black <- montgomery_tracts %>%
  filter(majority_black) %>%
  summarise(geometry = st_union(geometry))

montgomery_union_nonblack <- montgomery_tracts %>%
  filter(!majority_black) %>%
  summarise(geometry = st_union(geometry))
```
...
```{r, message=F, warning=F}
montgomery_boundary_line = 
  st_intersection(st_boundary(montgomery_union_black), st_boundary(montgomery_union_nonblack))
```
...
```{r, message=F, warning=F}
ggplot() +
  geom_sf(data = montgomery_tracts, aes(fill = majority_black), 
          color = "grey40", 
          alpha = 0.5) +
  geom_sf(data = montgomery_boundary_line, 
          color = "red", 
          size = 1) +
  labs(title = "Dividing Walls Between Majority Black and Non-Black Areas in Montgomery County, MD") +
  theme_minimal()
```

From an analysis of history regarding the region, and contextual knowledge about the urban make-up of Montgomery, MD, which is the minute proportion of the area, there is no clear evidence of white flight given the shape of the dividing wall.

Where might we go from here? Prince George county?

### PG County, MD

I will select another county: Price George County, MD for further investigation.
...
We can calculate the proportion of Black people in all PG tracts.
...
Approximately 56 percent of tracts have a Black population greater than or equal to 60 percent in MD.

Finally, we calculate $D$ for the county.

```{r, message=F, warning=F}
pgc_total_black <- sum(pgc_tracts$blackE, na.rm = TRUE)
pgc_total_nonblack <- sum(pgc_tracts$nonblackE, na.rm = TRUE)

dissimilarity_pgc <- 0.5 * sum(abs(
  (pgc_tracts$blackE / total_black) - 
  (pgc_tracts$nonblackE / total_nonblack)
), na.rm = TRUE)
```
...
Values between roughly 0.3 to 0.6 indicate moderate segregation; above 0.6 is high segregation. In this instance, the model result suggests low residential segregation by race in PG County. We close by visualizing our result and the potential dividing wall.
...
```{r, message=F, warning=F}
# Union geometries by group
pgc_union_black <- pgc_tracts %>%
  filter(majority_black) %>%
  summarise(geometry = st_union(geometry))

pgc_union_nonblack <- pgc_tracts %>%
  filter(!majority_black) %>%
  summarise(geometry = st_union(geometry))
```
...
```{r, message=F, warning=F}
pgc_boundary_line = 
  st_intersection(st_boundary(pgc_union_black), st_boundary(pgc_union_nonblack))
```
...
```{r, message=F, warning=F}
ggplot() +
  geom_sf(data = pgc_tracts, aes(fill = majority_black), 
          color = "grey40", 
          alpha = 0.5) +
  geom_sf(data = pgc_boundary_line, 
          color = "red", 
          size = 1) +
  labs(title = "Dividing Walls Between Majority Black and Non-Black Areas in PG County, MD") +
  theme_minimal()
```
PG county depicts a dividing wall much better the Montgomery county as PG county is predominantly black.
