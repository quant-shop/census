...
In the conceptual analysis above, there were only two tracts present and they completely segregated the two groups by the dividing wall. This conceptual model is a base example: two groups, two tracts, complete segregation.
...
ggplot() +
  ...
  geom_sf(data = boundary_line, 
          color = "red", 
          size = 1) +
  labs(title = "Dividing Wall Boundary Estimate for DC") +
  theme_minimal()
```
...
Similarly, for GA, we'll use `geography = "county"` and then make our way down to `tracts`.
...
We see that Hancock County, GA has the highest proportion of Black people in the state, but only by a small margin. Alternatively, in the code below, we see that Pickens, Murray, Fannin, and Gilmer counties have the lowest proportion of Black people in the state.
...
ggplot(fulton_tracts) +
  ...
  scale_fill_viridis_c(option = "magma", 
                       na.value = "grey50", 
                       labels = comma) +
  labs(title = "Estimated Black Population by Census Tract in Fulton County, GA (2023)",
       fill = "Population") +
  theme_minimal()
```
...
ggplot(fulton_tracts) +
  ...
  scale_fill_viridis_c(option = "magma", 
                       na.value = "grey50",
                       labels = comma) +
  labs(title = "Estimated Non-Black Population by Census Tract in Fulton County (2023)",
       fill = "Population") +
  theme_minimal()
```
...
fulton_tracts %>%
  ...
  summarise(
    total_tracts = n(),
    tracts_above_threshold = sum(proportion_black >= 0.90, na.rm = TRUE),
    proportion_above_threshold = mean(proportion_black >= 0.90, na.rm = TRUE)
  )
```

Approximately 16 percent of tracts have a Black population greater than or equal to 90 percent in Fulton County, GA.

Finally, we calculate $D$ for the county.

```{r, message=F, warning=F}
fulton_total_black <- sum(fulton_tracts$blackE, na.rm = TRUE)
fulton_total_nonblack <- sum(fulton_tracts$nonblackE, na.rm = TRUE)

dissimilarity_fulton <- 0.5 * sum(abs(
  (fulton_tracts$blackE / total_black) - 
  (fulton_tracts$nonblackE / total_nonblack)
), na.rm = TRUE)
```
...
ggplot(fulton_tracts) +
  geom_sf(aes(fill = proportion_black), color = "white") +
  scale_fill_viridis_c(option = "plasma", direction = -1) +
  labs(title = "Proportion of Black Residents by Census Tract in Fulton County, GA",
       fill = "Proportion Black") +
  theme_minimal()
```
...
```{r, message=F, warning=F}
# Union geometries by group
fulton_union_black <- fulton_tracts %>%
  filter(majority_black) %>%
  summarise(geometry = st_union(geometry))

fulton_union_nonblack <- fulton_tracts %>%
  filter(!majority_black) %>%
  summarise(geometry = st_union(geometry))
```
...
```{r, message=F, warning=F}
fulton_boundary_line = 
  st_intersection(st_boundary(fulton_union_black), st_boundary(fulton_union_nonblack))
```
...
```{r, message=F, warning=F}
ggplot() +
  geom_sf(data = fulton_tracts, aes(fill = majority_black), 
          color = "grey40", 
          alpha = 0.5) +
  geom_sf(data = fulton_boundary_line, 
          color = "red", 
          size = 1) +
  labs(title = "Dividing Wall Boundary Estimate for Fulton County, GA") +
  theme_minimal()
```
