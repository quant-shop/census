...
atl_tract <- get_acs(
  geography = "tract",
  county = c("Fulton", "Cherokee", "Clayton", "Cobb", "DeKalb", 
             "Douglas", "Fayette", "Forsyth", "Gwinnett", "Henry",
             "Rockdale"),
...
st_write(atl_tract, "atl_tract_income.shp") 
...
ggplot(atl_tract) +
  ...
  geom_sf(data = atl_county, fill = NA, color = "white", linewidth = 0.2) +
  theme_minimal()
```
...
centroids_atl <- st_centroid(atl_tract)

# Extract centroid coordinates
coord_atl <- st_coordinates(centroids_atl)
head(coord_atl)

# Extracting polygons id 
id_atl_tract <- atl_tract$GEOID
```
...
coord_atl_clean <- coord_atl[complete.cases(coord_atl), ]
id_atl_tract_clean <- id_atl_tract[complete.cases(coord_atl)]

```

```{r distancem, warning=FALSE, message=FALSE}
# Two points are connected if they are within 10 km
dnb10 <- dnearneigh(x = coord_atl_clean, d1 = 0, d2 = 10, 
                    row.names = id_atl_tract_clean, longlat = TRUE)
class(dnb10) # nb = neighbor list

# Two points are connected if they are within 50 km
dnb50 <- dnearneigh(x = coord_atl_clean, d1 = 0, d2 = 15, 
                    row.names = id_atl_tract_clean, longlat = TRUE)
class(dnb50)
```
...
# k-nearest-neighbors weights matrix:
neighbors <- knearneigh(x = coord_atl_clean, k = 1, longlat = T)
...
k1.dist <-  nbdists(k1, coord_atl_clean, longlat = T)
...
dnb68 <- dnearneigh(x = coord_atl_clean, d1 = 0, d2 = all.linkedT, 
                    row.names = id_atl_tract_clean, longlat=TRUE)
# dnb68
```
...
par(mfrow=c(1, 3))
plot(dnb68, coord_atl_clean)
title("k-nearest-neighbors")
plot(dnb10, coord_atl_clean)
title("Distance-based matrix: 10 km")
plot(dnb50, coord_atl_clean)
title("Distance-based matrix: 15 km")
...
atl_clean <- atl_tract[!st_is_empty(atl_tract), ]
coords   <- st_coordinates(st_centroid(atl_clean))  # one row per feature
id       <- atl_clean$GEOID
y        <- atl_clean$incomeE
...
dnb10.listw <- nb2listw(dnb10, style = "W") # Row standardized matrix
dnb10.listw
...
moran.test(x = y, listw = dnb10.listw, randomisation = F, 
           zero.policy = F, alternative= "greater", na.action = na.fail)
  
# Moran scatterplot 
msp<- moran.plot(x = y, listw = dnb10.listw, 
                labels = F, 
                 pch = 2, 
                 main = "Moran's plot",
...
wx <- spdep::lag.listw(dnb10.listw, y, zero.policy = TRUE)  
...
plot(atl_tract$incomeE, col=c("gray", "yellow", "red", "green", "blue")[cols],axes=T)
...
id_vec <- if ("GEOID" %in% names(atl_clean)) atl_clean$GEOID else as.character(seq_len(nrow(atl_clean)))

# Build contiguity neighbors
nb <- spdep::poly2nb(atl_clean, queen = queen_contiguity, row.names = id_vec)
